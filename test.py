...
def parse_args():
    ...
    parser.add_argument(
        "--tta", action="store_true", help="Whether to use test time augmentation"
    )
    parser.add_argument("--show", action="store_false", help="show prediction results")
    parser.add_argument(
...
def main():
    ...
    if args.tta:
        assert "tta_model" in cfg, (
            "Cannot find ``tta_model`` in config." " Can't use tta !"
        )
        assert "tta_pipeline" in cfg, (
