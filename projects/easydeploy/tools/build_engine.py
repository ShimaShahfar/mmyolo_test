...
class EngineBuilder:
    ...
    def __build_engine(
        ...
        if dshape:
            ...
            if scale is None:
                scale = np.array(
                    [[1, 1, 0.5, 0.5], [1, 1, 1, 1], [4, 1, 1.5, 1.5]], dtype=np.float32
                )
        ...
        logger.log(
            trt.Logger.WARNING,
            f"Build tensorrt engine finish.\n" f"Save in {str(self.weight.absolute())}",
        )

    def build(
        self, scale: Optional[List[List]] = None, fp16: bool = True, with_profiling=True
    ):
...
def parse_args():
    ...
    parser.add_argument(
        "--scales",
        type=str,
        default="[[1,3,512,1024],[1,3,512,1024],[1,3,512,1024]]",
        help="Input scales for build dynamic input shape engine",
    )
    parser.add_argument(
        "--fp16", action="store_false", help="Build model with fp16 mode"
    )
