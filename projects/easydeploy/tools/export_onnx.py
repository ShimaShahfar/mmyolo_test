...
def parse_args():
    ...
    parser.add_argument("--model-only", action="store_true", help="Export model only")
    parser.add_argument(
    ...
    parser.add_argument(
        "--backend", type=str, default="onnxruntime", help="Backend for export onnx"
    )
    parser.add_argument(
        "--pre-topk",
        type=int,
    ...
    parser.add_argument(
        "--iou-threshold", type=float, default=0.6, help="IoU threshold for NMS"
    )
    parser.add_argument(
        "--score-threshold", type=float, default=0.25, help="Score threshold for NMS"
    )
...
def main():
    ...
    if backend in (
        ...
        print_log(
            f"Can not export postprocess for {args.backend.lower()}.\n"
    ...
    save_onnx_path = os.path.join(
        args.work_dir, os.path.basename(args.checkpoint).replace("pth", "onnx")
    )
    # export onnx
    with BytesIO() as f:
        if args.fp16:
        ...
        if not args.model_only and backend in (
            MMYOLOBackend.TENSORRT8,
            MMYOLOBackend.TENSORRT7,
        ):
            shapes = [
